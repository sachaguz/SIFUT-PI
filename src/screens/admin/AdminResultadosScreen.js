...
export default function AdminResultadosScreen({ navigation }) {
  ...
  const fetchPartidos = useCallback(() => {
    ...
    api.get(`/partidos?torneoId=${torneoId}`).then((r) => setPartidos(r.data)).finally(() => setLoadingPartidos(false));
  }, [torneoId]);
  ...
  const handleEliminar = (partido) => {
    Alert.alert('Eliminar partido', `¿Eliminar ${partido.equipoLocal?.nombre} vs ${partido.equipoVisitante?.nombre}?`, [
      { text: 'Cancelar', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            ...
            Alert.alert('Error', err.response?.data?.error || 'No se pudo eliminar el partido.');
          }
        },
      },
    ]);
  ...
  const jornadaNumeros = [...new Set(partidos.filter((p) => p.fase === 'JORNADA').map((p) => p.jornada))].sort((a, b) => a - b);
  const fasesPresentes = ['CUARTOS', 'SEMIFINAL', 'FINAL'].filter((f) => partidos.some((p) => p.fase === f));
  const jornadaLabels = [...jornadaNumeros.map((n) => `Jornada ${n}`), ...fasesPresentes.map(faseLabel)];

  const activeLabel = jornadaLabels.includes(jornadaSel) ? jornadaSel : (jornadaLabels[0] || '');
  ...
  return (
    <ScreenContainer>
      ...
      {loadingPartidos ? (
        ...
        <>
          <PillSelector label="Jornada" options={jornadaLabels} value={activeLabel} onChange={setJornadaSel} />
          ...
          {partidosMostrados.map((partido) => {
            ...
            return (
              <ListRow
                ...
                title={finalizado ? `${local} ${partido.golesLocal}-${partido.golesVisitante} ${visitante}` : `${local} vs ${visitante}`}
                ...
                right={(
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.sm }}>
                    <Badge label={estadoLabel(partido.estado)} tone={finalizado ? 'success' : 'warning'} />
                    <TouchableOpacity
                      hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
                      onPress={() => navigation.navigate('AdminPartidoForm', { partido: { ...partido, fecha: partido.fecha } })}
                    >
                      <Ionicons name="create-outline" size={20} color={colors.textMuted} />
                    </TouchableOpacity>
                    <TouchableOpacity
                      hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
                      onPress={() => handleEliminar(partido)}
                    >
                ...
                onPress={() => finalizado
                  ? navigation.navigate('PartidoDetalle', { partido: { ...partido, local, visitante, torneo, cancha, fecha: formatDate(partido.fecha) } })
                  : navigation.navigate('AdminRegistroResultado', { partido: { ...partido, local, visitante, torneo, cancha, fecha: formatDate(partido.fecha) } })
                }
              />
            );
          })}
      ...
      <View style={{ marginTop: spacing.md }}>
        <PrimaryButton title="+ Programar partido" onPress={() => navigation.navigate('AdminPartidoForm', { torneoId })} />
      </View>
    </ScreenContainer>
  );
}
