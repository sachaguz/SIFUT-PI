...
export default function AdminEquiposScreen({ navigation }) {
  ...
  useFocusEffect(
    useCallback(() => {
      Promise.all([api.get('/equipos'), api.get('/torneos')])
        .then(([e, t]) => {
          setEquipos(e.data);
          setTorneos(t.data);
        })
  ...
  return (
    <ScreenContainer>
      <SectionHeader eyebrow="Ligas" title="Equipos" subtitle={`${equipos.length} equipos registrados`} />
      ...
      <View style={{ marginTop: spacing.md }}>
        <PrimaryButton title="+ Nuevo equipo" onPress={() => navigation.navigate('AdminEquipoForm')} />
      </View>
    </ScreenContainer>
  );
}
